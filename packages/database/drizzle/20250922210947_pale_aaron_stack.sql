...
ALTER TYPE "public"."notification_type" ADD VALUE 'LEADERBOARD_RANK_UPDATE';--> statement-breakpoint
...
-- Skip criteria conversion if already JSON type
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'Badge' AND column_name = 'criteria' AND data_type = 'json'
    ) THEN
        RAISE NOTICE 'Badge.criteria is already JSON type, skipping conversion';
    ELSE
        ALTER TABLE "Badge" ALTER COLUMN "criteria" SET DATA TYPE json USING CASE
          WHEN "criteria" IS NULL OR "criteria" = '' THEN NULL
          WHEN "criteria"::text ~ '^\\s*\\{' THEN "criteria"::json
          WHEN "criteria"::text ~ '^\\s*\\[' THEN "criteria"::json
          ELSE json_build_object('value', "criteria")
        END;
    END IF;
